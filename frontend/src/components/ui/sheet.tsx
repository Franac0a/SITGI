  ...
  'fixed z-50 gap-4 bg-white p-6 shadow-lg transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-500 data-[state=open]:animate-in data-[state=closed]:animate-out',
  {
    variants: {
      side: {
        ...
        left: 'inset-y-0 left-0 h-full w-72 sm:w-80 border-r data-[state=closed]:slide-out-to-left data-[state=open]:slide-in-from-left p-0',
        right:
...
>(({ side = 'left', className, children, hideCloseButton = false, ...props }, ref) => (
  <SheetPortal>
    ...
    >
      {children}
      {!hideCloseButton && (
        <SheetPrimitive.Close className="absolute right-4 top-4 rounded-sm opacity-70 ring-offset-white transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-cit-turquesa focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-gray-100">
